...
class EmbeddingStore:
    ...
    def _make_record(self, doc: Document) -> dict[str, Any]:
        ...
        # Shallow-copy metadata so mutating the stored record never mutates the caller's Document.
        meta = dict(doc.metadata) if doc.metadata else {}
        # Ensure doc_id is always resolvable from metadata (needed by delete_document),
        # even when the caller didn't set it explicitly (e.g. ingest.py already does).
    ...
    def _search_records(self, query: str, records: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
        """Run in-memory similarity search (dot product) over the given records."""
        if not records:
            return []
